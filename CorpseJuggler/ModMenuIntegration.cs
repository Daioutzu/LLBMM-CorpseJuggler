    ...
    public class ModMenuIntegration : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (!Directory.Exists(iniLocation))
            {
                ...
                Debug.Log("[LLBMM] CorpseJuggler: Created ModSettings Folder");
            }
            if (!File.Exists(iniLocation + gameObject.name + ".ini"))
            {
                InitConfig();
                Debug.Log($"[LLBMM] CorpseJuggler: {gameObject.name}.ini Created in {iniLocation}");
            }
            ReadIni();
        }
        private void OnDestroy()
        {
            Debug.Log("[LLBMM] CorpseJuggler: ModMenuIntegration Destroyed");
        }
        ...
        private void InitConfig()
        {
            ...
            */
            ...
            AddToWriteQueue("(key)enableCorpseJugglerKey", "B");
            AddToWriteQueue("(key)spawnCorpseKey", "Q");
            AddToWriteQueue("(key)enableBallPositioningKey", "Mouse1");

            AddToWriteQueue("(header)headerName1", "Controller Settings");
            AddToWriteQueue("(bool)enableCorpseSpawnerWithSelect", "true");
            AddToWriteQueue("(bool)SpawnCorpseWithRightStickDown", "true");
            AddToWriteQueue("(bool)enableBallPositioningWithRightStickPressed", "true");
            AddToWriteQueue("(slider)ballSpeedSlider", "21|1|33");

            AddToWriteQueue("(gap)datDap", "soTastyMuchWow");
            AddToWriteQueue("(header)headerName2", "UI Elements");
            AddToWriteQueue("(bool)In-GameCredit", "true");
